...
function TurfDetail() {
  ...
  const handleShow = (event) => {
    // Booking logic here
    // IMPORTANT: Avoid using alert() in production applications, use a custom modal or toast
    console.log(`Booking for: ${event.name}`);
    alert(`Booking for: ${event.name}`); // Keeping for now as per original code, but consider replacing
  };
        ...
        <Card.Body>
          <Row>
            ...
            <p><strong>Address:</strong> {turfData.address}</p>
              <p><strong>Price:</strong> ₹{turfData.price}</p>
              {turfData.heroimg && (
                // Correctly constructing the image URL for locally stored images
                <img
                  src={`${import.meta.env.VITE_BACKEND_BASE_URL}/${turfData.heroimg}`}
                  alt="Turf"
                  className="img-fluid rounded shadow-sm"
                  ...
                  <Col md={6} lg={3} key={idx} className="mb-4">
                    <Card className="h-100 shadow-sm">
                      ...
                      <Card.Body className="d-flex flex-column">
                        ...
                        <Card.Text>Type: {event.type}</Card.Text>
                        <Card.Text>Price: ₹{event.price}/hour</Card.Text>
                        {/* Optional: Add a booking button here */}
                        {/* <Button variant="success" onClick={() => handleShow(event)} className="mt-auto">
                          Book Event
                        </Button> */}
                      </Card.Body>
                    </Card>
