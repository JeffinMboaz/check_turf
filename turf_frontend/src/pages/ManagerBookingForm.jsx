...
//       const response = await axios.get('${import.meta.env.Backend_Base_Url}/api/auth/getmgrbooking', {
...
const ManagerBookings = () => {
  ...
  const fetchTurfs = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`${import.meta.env.Backend_Base_Url}/api/auth/mngrturfs`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setTurfs(response.data);
    } catch (err) {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      ...
      try {
        const token = localStorage.getItem('token');
        const response = await axios.get(`${import.meta.env.Backend_Base_Url}/api/auth/getevents/${selectedTurf._id}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        setEvents(response.data.events || []);
      } catch (err) {
  ...
  const cancelBooking = async (bookingId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(`${import.meta.env.Backend_Base_Url}/api/auth/userCancelBooking/${bookingId}`, {}, {
        headers: { Authorization: `Bearer ${token}` },
      });
      toast.success("Booking cancelled successfully!");
      fetchBookings();
  ...
  const handleBookingSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post(`${import.meta.env.Backend_Base_Url}/api/auth/mgrbookturf`, formData, {
        headers: { Authorization: `Bearer ${token}` },
      });
      toast.success("Turf booked successfully!");
      setShowForm(false);
