...
//       const res = await axios.get("http://localhost:5006/api/auth/admgetallbookings", {
//         headers: { Authorization: `Bearer ${token}` },
//       });
//       setBookings(res.data); // changed from res.data.data
//     } catch (err) {
//       console.error(err);
...
function ManageBookings() {
  ...
  const handleCancel = async (bookingId) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.delete(`http://localhost:5006/api/auth/admcancelbook/${bookingId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
