...
const [newEventData, setNewEventData] = useState({
    ...
    const handleAddEvent = async () => {
        ...
        try {
            await axios.post(`http://localhost:5006/api/auth/addevent/${editTurfData._id}`, 
                {
                    turfId: editTurfData._id,
                    ...newEventData
                }, 
                {
                    headers: { Authorization: `Bearer ${token}` }
            ...
            handleTurfSelect(editTurfData._id); // refresh event list
        } catch (err) {
        ...
        <>
            ...
            <Button className='btn btn-success mx-2' onClick={() => setShowAddModal(true)}>New turf</Button>
                ...
                {/* <div className='mb-4 w-25'>
                    <select className='form-select' onChange={(e) => handleTurfSelect(e.target.value)}>
                    ...
                    <Form>
                        ...
                        <Form.Group className="mb-3">
                            ...
                            <Form.Control
                                type="text"
                                name="img"
                                value={editEventData.img}
                                onChange={(e) => setEditEventData({ ...editEventData, img: e.target.value })}
                            />
                        </Form.Group>
                    </Form>
                </Modal.Body>
 ...
 <Modal show={showAddModal} onHide={() => setShowAddModal(false)} size="lg">
  ...
  <Modal.Body style={{ maxHeight: '70vh', overflowY: 'auto' }}>
    <Form>
      ...
      <Form.Group className="mb-3">
        ...
        <Form.Control
          type="text"
          value={newTurfData.coordinates}
          onChange={(e) => setNewTurfData({ ...newTurfData, coordinates: e.target.value })}
        />
      </Form.Group>
      ...
      <Form.Group className="mb-3">
        ...
        <Form.Control
          type="text"
          value={newTurfData.heroimg}
          onChange={(e) => setNewTurfData({ ...newTurfData, heroimg: e.target.value })}
        />
      </Form.Group>
  ...
  <Modal.Footer>
    ...
    <Button variant="success" onClick={async () => {
      try {
        const coords = newTurfData.coordinates.split(',').map(Number);
        const payload = {
          ...newTurfData,
          coordinates: coords,
        };
        await axios.post('http://localhost:5006/api/auth/addturf', payload, {
          headers: { Authorization: `Bearer ${token}` },
        });
        alert("Turf added successfully!");
        setShowAddModal(false);
        window.location.reload(); // or refetch turfs
      } catch (err) {
        console.error(err);
        ...
        <Form>
            ...
            <Form.Group className="mb-3">
                ...
                <Form.Control
                    ...
                    onChange={(e) => setNewEventData({ ...newEventData, img: e.target.value })}
                />
            </Form.Group>
        </Form>
    </Modal.Body>
