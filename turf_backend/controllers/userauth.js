...
//     try {
//         const { fullname, email, phonenumber, password, targetUserId } = req.body;
...
//                 || await Manager.findById(targetUserId)
//                 || await Admin.findById(targetUserId);
//         } else {
...
//             user.password = await bcrypt.hash(password.toLowerCase(), salt);
//         }
...
const updateUserProfile = async (req, res) => {
  try {
    ...
    console.error("Update error:", error); // ✅ This line is important
    res.status(500).json({ message: "Server error" });
  }
};
